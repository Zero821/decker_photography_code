		...
		<transition appear name="slide-trans">
			<div>
				...
				<div class="gallery align-items-baseline" :class="{blur: displayOverlay}">
					<GalleryGrid @showFull='displayFull($event)' v-for="photo in Photos.landscapes" :key="photo.id" :photo="photo" :year="'landscapes'"/>
				</div>
        <transition appear name="drop">
				  <Fullsize v-if="displayOverlay" :photos="Photos" :file="source" :location="'landscapes'" @click="hideOverlay"/>
        </transition>
      </div>
		</transition>
	</div>
