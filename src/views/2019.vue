		...
		<transition appear name="slide-trans">
			<div>
				...
				<div class="gallery align-items-baseline" :class="{blur: displayOverlay}">
					<GalleryGrid @showFull='displayFull($event)' v-for="photo in Photos.nineteen" :key="photo.id" :photo="photo" :year="2019"/>
				</div>
        <transition appear name="drop">
				  <Fullsize v-if="displayOverlay" :photos="Photos" :file="source" :year="2019" @click="hideOverlay"/>
        </transition>
      </div>
		</transition>
	</div>
